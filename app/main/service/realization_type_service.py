...
def update_realization_types(data):
    ...
    for realizationType in data:
        new_query = query.filter(RealizationType.id == realizationType['id'])
        record = new_query.one()
        record.realizationTypeEn = realizationType["realizationTypeEn"]
        record.realizationTypeFr = realizationType["realizationTypeFr"]
        record.displayOrder = realizationType["displayOrder"]
    db.session.flush()
    db.session.commit()
    response_object = {
        'status': 'success',
